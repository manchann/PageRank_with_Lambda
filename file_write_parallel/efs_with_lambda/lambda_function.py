...
def put_dynamodb_items(case, data):
    table.put_item(
        Item={
            'case': case,
            '1_before_file_open': decimal.Decimal(str(data[0])),
            '2_before_file_write': decimal.Decimal(str(data[1])),
            '3_after_file_write': decimal.Decimal(str(data[2])),
            '4_after_file_close': decimal.Decimal(str(data[3]))
        }
    )
